...
function ctable_shortcode_preview($atts)
{
	...
	<div class="ctable-pricing-table">
		<?php $ctable_user_roles = $wpdb->get_results(
      $wpdb->prepare(
          "SELECT id,role FROM {$wpdb->prefix}ctable_user GROUP BY role ORDER BY id ASC"
      ),
      ARRAY_A
  ); ?>
		<div class="roles-box">
        ...
        $singlerole["role"]
    ); ?>"><?php echo esc_html($singlerole["role"]); ?></option>
				<?php } ?>
			</select>
		</div>
